...
func (bs *BlockStore) SaveBlocks(new_blocks []*protocol.Block, endHeight int64) error {
	...
	for _, block := range new_blocks {
		startHeight := (block.BlockHeight / BLOCKS_PER_FILE) * BLOCKS_PER_FILE
	...
	for startHeight, blocks := range bs.blockMap {
		...
		if block_count >= BLOCKS_PER_FILE || (block_count > 0 && blocks[block_count-1].BlockHeight == endHeight) {
			...
			// Generate file path
			endHeight := min(startHeight+BLOCKS_PER_FILE-1, endHeight)
